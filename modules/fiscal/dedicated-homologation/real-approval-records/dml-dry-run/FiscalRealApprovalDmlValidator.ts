import { FiscalRealApprovalDmlDryRunInput } from './FiscalRealApprovalDmlDryRunTypes';

export class FiscalRealApprovalDmlValidator {
  public static validate(input: FiscalRealApprovalDmlDryRunInput) {
    ...
    if (jsonStr.includes('BEGIN')) blockers.push('BEGIN blocked');
    if (jsonStr.includes('TRANSACTION')) blockers.push('TRANSACTION blocked');
    ...
    if (input.forceSeedRealData) blockers.push('forceSeedRealData blocked');
    if (input.forceExecuteDml) blockers.push('forceExecuteDml blocked');
    if (input.forceInsert) blockers.push('forceInsert blocked');
    if (input.forceUpdate) blockers.push('forceUpdate blocked');
    if (input.forceDelete) blockers.push('forceDelete blocked');
