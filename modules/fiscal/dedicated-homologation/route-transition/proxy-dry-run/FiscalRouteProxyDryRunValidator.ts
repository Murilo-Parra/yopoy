import { FiscalRouteProxyDryRunInput } from './FiscalRouteProxyDryRunTypes';

export class FiscalRouteProxyDryRunValidator {
  public static validate(input: FiscalRouteProxyDryRunInput) {
    ...
    if (lowerJson.includes('res.send') || lowerJson.includes('res.json')) {
       ...
       // simpler validation since the rules explicitly ask to block "res.send override", not any res.send usage
    ...
    const forceKeys = [
      ...
      'forceModifyAppUse', 'forceModifyRouterUse', 'forceRouteToV2',
      'forceDisableLegacyRoute', 'forceChangeTraffic', 'forceCaptureRequest',
      'forceCaptureResponse', 'forceCallV2Handler', 'forceCallLegacyHandler',
      'forceActivateProductionV2', 'forceActivateRelease', 'forceActivateCanary',
