import { FiscalProductionRolloutApprovalInput } from './FiscalProductionRolloutApprovalTypes';

export class FiscalProductionRolloutApprovalValidator {
  public static validate(input: FiscalProductionRolloutApprovalInput) {
    ...
    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('approval') && !lowerJson.includes('verification') && !lowerJson.includes('readiness') && !lowerJson.includes('no-op') && !lowerJson.includes('noop') && !lowerJson.includes('matrix') && !lowerJson.includes('plan') && !lowerJson.includes('review') && !lowerJson.includes('evidence')) {
    ...
    const forceKeys = [
      'forceApproveRealRollout', 'forceExecuteRealRollout', 'forceActivateRealRelease', 'forceActivateRealCanary',
      'forceExecuteGoLive', 'forcePromoteRealTraffic', 'forceActivateProductionV2', 'forceChangeTraffic', 'forceRouteToV2',
      'forceDisableLegacyRoute', 'forceInstallProxy', 'forceInstallMiddleware', 'forceInstallTap', 'forceInstallMirror',
      'forceInstallSniffer', 'forceEnableShadowTraffic', 'forceMirrorRealTraffic', 'forceDuplicateRequest', 'forceCaptureRequest',
      'forceCaptureResponse', 'forceCapturePayload', 'forceModifyAppUse', 'forceModifyRouterUse', 'forceCallRealEndpoint',
      'forceCallLegacyHandler', 'forceCallV2Handler', 'forceStartRuntimeExecution', 'forceStartCommandQueue', 'forceUnlockRealQueue',
      'forceEnqueueRealJob', 'forceDispatchRealWorker', 'forceCreateWorker', 'forceCreateScheduler', 'forceCreateCron',
      'forceRunCommandRunner', 'forceExecuteShellCommand', 'forceOpenRealTransaction', 'forceCommitRealTransaction',
      'forceRollbackRealTransaction', 'forceConnectRealDatabase', 'forceExecuteDml', 'forceExecuteDdl', 'forceCallRealSefaz',
      ...
      'forceApproveRealDeploy', 'forceExecuteRealDeploy', 'forceExecuteRealRelease', 'forceApproveRealCutover',
      'forceExecuteRealCutover', 'forceExecuteRealRollback', 'forcePublishRealPackage', 'forceGenerateExecutableArtifact'
    ];
