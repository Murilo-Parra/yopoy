import { FiscalProductionExecutionAuthorizationClosureInput } from './FiscalProductionExecutionAuthorizationClosureTypes';

export class FiscalProductionExecutionAuthorizationClosureValidator {
  public static validate(input: FiscalProductionExecutionAuthorizationClosureInput) {
    ...
    const kwToBlock = [
      ...
      'committeeapproval', 'persistdeliberation', 'approvalrecord', 'votingrecord',
      'finalauthorization', 'authorizationpackage', 'gatehandoff',
      'closureapproval', 'productionexecutionclosure'
    ];

    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          const isExceptedKeyword = kw === 'closureapproval' || kw === 'productionexecutionclosure';
          if (isExceptedKeyword) {
             blockers.push(`${kw} blocked`);
          } else if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('simulation') && !lowerJson.includes('plan') && !lowerJson.includes('closure') && !lowerJson.includes('evidence') && !lowerJson.includes('blueprint') && !lowerJson.includes('boundary') && !lowerJson.includes('intake') && !lowerJson.includes('sanitizer') && !lowerJson.includes('envelope')) {
             blockers.push(`${kw} blocked`);
          }
       }
    }
    ...
    const forceKeys = [
      'forceGrantRealAuthorization', 'forceUnlockRealGate',
      'forcePersistRealAuthorizationPackage', 'forcePersistRealApprovalRecord', 'forceNotifyExternalStakeholder',
      ...
      'forceEnableShadowTraffic', 'forceCreateWorker', 'forceCreateScheduler', 'forceConnectRealDatabase', 'forceExecuteDml',
