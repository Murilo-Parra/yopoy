import { FiscalProductionRollbackAbortInput } from './FiscalProductionRollbackAbortTypes';

export class FiscalProductionRollbackAbortValidator {
  public static validate(input: FiscalProductionRollbackAbortInput) {
    ...
    const kwToBlock = [
      'canary', 'cutover', 'rollback', 'abortcutover', 'switchtraffic', 'activatev2', 'disablelegacy',
    ...
    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('simulation') && !lowerJson.includes('noop') && !lowerJson.includes('no-op') && !lowerJson.includes('plan') && !lowerJson.includes('catalog') && !lowerJson.includes('evidence') && !lowerJson.includes('matrix') && !lowerJson.includes('checklist')) {
    ...
    const forceKeys = [
      'forceExecuteRealRollback', 'forceExecuteRealAbort', 'forceApproveRealCutover', 'forceExecuteRealCutover',
      'forceExecuteGoLive', 'forceActivateProductionV2', 'forceChangeTraffic', 'forceRouteToV2', 'forceDisableLegacyRoute',
      ...
      'forceCallLegacyHandler', 'forceCallV2Handler', 'forceStartRuntimeExecution', 'forceStartCommandQueue',
      'forceUnlockRealQueue', 'forceEnqueueRealJob', 'forceDispatchRealWorker', 'forceCreateWorker', 'forceCreateScheduler',
      'forceCreateCron', 'forceRunCommandRunner', 'forceExecuteShellCommand', 'forceOpenRealTransaction',
      'forceCommitRealTransaction', 'forceRollbackRealTransaction', 'forceConnectRealDatabase', 'forceExecuteDml',
      'forceExecuteDdl', 'forceCallRealSefaz', 'forceLoadRealCertificate', 'forceReadRealPfx', 'forceReadCertificatePassword',
      'forceUseRealCrypto', 'forceSignRealXml', 'forceGenerateRealPdf', 'forceExecuteProduction', 'forceGrantRealAuthorization',
      'forceUnlockRealGate', 'forceApproveRealDeploy', 'forceExecuteRealDeploy', 'forceExecuteRealRelease',
      'forceExecuteRealRollout', 'forceActivateRealCanary', 'forcePublishRealPackage', 'forceGenerateExecutableArtifact'
    ];
