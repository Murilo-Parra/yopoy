import { FiscalProductionExecutionBoundaryInput } from './FiscalProductionExecutionBoundaryTypes';

export class FiscalProductionExecutionBoundaryValidator {
  public static validate(input: FiscalProductionExecutionBoundaryInput) {
    ...
    for (const kw of keywords) {
      if (lowerJson.includes(kw)) {
        if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('simulation') && !lowerJson.includes('plan') && !lowerJson.includes('closure') && !lowerJson.includes('evidence') && !lowerJson.includes('blueprint') && !lowerJson.includes('boundary')) {
    ...
    const forceKeys = [
      'forceUnlockRealGate', 'forceGrantRealAuthorization', 'forceApproveRealDeploy', 'forceExecuteRealDeploy',
