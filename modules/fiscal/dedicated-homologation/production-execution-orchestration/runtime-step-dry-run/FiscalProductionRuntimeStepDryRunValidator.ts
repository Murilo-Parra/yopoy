import { FiscalProductionRuntimeStepDryRunInput } from './FiscalProductionRuntimeStepDryRunTypes';

export class FiscalProductionRuntimeStepDryRunValidator {
  public static validate(input: FiscalProductionRuntimeStepDryRunInput) {
    ...
    const kwToBlock = [
      ...
      'orchestrationrun', 'productionexecution'
    ];

    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('simulation') && !lowerJson.includes('plan') && !lowerJson.includes('closure') && !lowerJson.includes('evidence') && !lowerJson.includes('blueprint') && !lowerJson.includes('boundary') && !lowerJson.includes('intake') && !lowerJson.includes('sanitizer') && !lowerJson.includes('envelope') && !lowerJson.includes('orchestration') && !lowerJson.includes('manifest')) {
    ...
    const forceKeys = [
      'forceStartRuntimeExecution', 'forceStartCommandQueue', 'forceCreateWorker',
