...
export class FiscalSandboxPersistenceValidator {
  ...
  public static validate(input: any): { valid: boolean; blockers: string[]; safeInput: any } {
    ...
    if (input.route.toLowerCase().includes('critical') && input.operation.toLowerCase() === 'emit') {
       blockers.push('Route attempting to simulate real emission is BLOCKED');
    }

    // Force safe shape and boolean guards
    const safeInput = {
      ...input,
      sandboxOnly: true,
      ...
      status: blockers.length > 0 ? FiscalSandboxSnapshotStatus.BLOCKED : (input.status || FiscalSandboxSnapshotStatus.STORED),
    };
  ...
  private static checkForSensitiveData(obj: any, blockers: string[]) {
    ...
    if (lowerObj.includes('payload') && obj.payload !== undefined) blockers.push('Raw payload found in shape');
  }
}
