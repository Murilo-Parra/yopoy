import { FiscalProductionRuntimeExecutionGraphInput } from './FiscalProductionRuntimeExecutionGraphTypes';

export class FiscalProductionRuntimeExecutionGraphValidator {
  public static validate(input: FiscalProductionRuntimeExecutionGraphInput) {
    ...
    const kwToBlock = [
      ...
      'executiongraph', 'executegraph', 'opentransaction', 'committransaction', 'rollbacktransaction',
      'dbtransaction', 'sefazcall', 'signxml', 'generatepdf', 'cryptosign', 'idempotencywrite',
      'productionexecution', 'golive', 'unlockgate', 'grantauthorization', 'executeproduction'
    ];

    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('plan') && !lowerJson.includes('noop') && !lowerJson.includes('no-op') && !lowerJson.includes('evidence') && !lowerJson.includes('checkpoint') && !lowerJson.includes('simulation') && !lowerJson.includes('boundary')) {
    ...
    const forceKeys = [
      'forceExecuteRuntimeGraph', 'forceStartRuntimeExecution', 'forceStartCommandQueue', 'forceUnlockRealQueue',
      ...
      'forceRollbackRealTransaction', 'forceConnectRealDatabase', 'forceExecuteDml', 'forceExecuteDdl',
      'forceCallRealSefaz', 'forceLoadRealCertificate', 'forceReadRealPfx', 'forceReadCertificatePassword',
      'forceUseRealCrypto', 'forceSignRealXml', 'forceGenerateRealPdf', 'forceExecuteProduction',
      'forceGrantRealAuthorization', 'forceUnlockRealGate', 'forceApproveRealDeploy', 'forceExecuteRealDeploy',
      'forceExecuteRealRelease', 'forceExecuteRealRollout', 'forceActivateRealCanary', 'forceApproveRealCutover',
      ...
      'forceInstallProxy', 'forceInstallMiddleware', 'forceInstallTap', 'forceModifyAppUse', 'forceModifyRouterUse',
      'forceCallRealEndpoint', 'forceCallLegacyHandler', 'forceCallV2Handler', 'forceCaptureRequest',
      'forceCaptureResponse', 'forceCapturePayload', 'forceDuplicateRequest', 'forceMirrorRealTraffic',
      'forceEnableShadowTraffic'
    ];
