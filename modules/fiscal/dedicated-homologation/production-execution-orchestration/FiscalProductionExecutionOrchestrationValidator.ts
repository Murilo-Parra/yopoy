import { FiscalProductionExecutionOrchestrationInput } from './FiscalProductionExecutionOrchestrationTypes';

export class FiscalProductionExecutionOrchestrationValidator {
  public static validate(input: FiscalProductionExecutionOrchestrationInput) {
    ...
    const kwToBlock = [
      ...
      'unlockgate', 'grantauthorization', 'executeproduction', 'productionexecution',
      'runtimeexecution', 'orchestrationrun', 'startworker', 'queue.process',
      'setinterval', 'scheduler', 'cron'
    ];

    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('simulation') && !lowerJson.includes('plan') && !lowerJson.includes('closure') && !lowerJson.includes('evidence') && !lowerJson.includes('blueprint') && !lowerJson.includes('boundary') && !lowerJson.includes('intake') && !lowerJson.includes('sanitizer') && !lowerJson.includes('envelope') && !lowerJson.includes('orchestration')) {
    ...
    const forceKeys = [
      'forceExecuteProduction', 'forceGrantRealAuthorization', 'forceUnlockRealGate',
