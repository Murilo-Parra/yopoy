import { FiscalProductionCutoverClosureInput } from './FiscalProductionCutoverClosureTypes';

export class FiscalProductionCutoverClosureValidator {
  public static validate(input: FiscalProductionCutoverClosureInput) {
    ...
    const kwToBlock = [
      'canary', 'cutover', 'rollback', 'rollout', 'promotetraffic', 'switchtraffic', 'activatev2', 'disablelegacy',
      'mirrortraffic', 'shadowtraffic', 'duplicaterequest', 'captureresponse',
      'deploy', 'releaseproduction', 'activateproduction',
    ...
    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('closure') && !lowerJson.includes('inventory') && !lowerJson.includes('checklist') && !lowerJson.includes('evidence') && !lowerJson.includes('handoff') && !lowerJson.includes('roadmap')) {
    ...
    const forceKeys = [
      'forceCloseAsRealCutover', 'forceGrantFinalGoLive', 'forceApproveRealRollout', 'forceExecuteRealRollout',
      'forceActivateRealRelease', 'forceActivateRealCanary', 'forceExecuteGoLive', 'forcePromoteRealTraffic',
      'forceExecuteRealRollback', 'forceExecuteRealAbort', 'forceExecuteRealRerouting', 'forceApproveRealCutover',
      'forceExecuteRealCutover', 'forceActivateProductionV2', 'forceChangeTraffic', 'forceRouteToV2',
      'forceDisableLegacyRoute', 'forceInstallProxy', 'forceInstallMiddleware', 'forceInstallTap',
      'forceInstallMirror', 'forceInstallSniffer', 'forceEnableShadowTraffic', 'forceMirrorRealTraffic',
      'forceDuplicateRequest', 'forceCaptureRequest', 'forceCaptureResponse', 'forceCapturePayload',
      'forceModifyAppUse', 'forceModifyRouterUse', 'forceCallRealEndpoint', 'forceCallLegacyHandler',
      'forceCallV2Handler', 'forceStartRuntimeExecution', 'forceStartCommandQueue', 'forceUnlockRealQueue',
      'forceEnqueueRealJob', 'forceDispatchRealWorker', 'forceCreateWorker', 'forceCreateScheduler',
      'forceCreateCron', 'forceRunCommandRunner', 'forceExecuteShellCommand', 'forceOpenRealTransaction',
      'forceCommitRealTransaction', 'forceRollbackRealTransaction', 'forceConnectRealDatabase',
      ...
      'forceGenerateRealPdf', 'forceExecuteProduction', 'forceGrantRealAuthorization', 'forceUnlockRealGate',
      'forceApproveRealDeploy', 'forceExecuteRealDeploy', 'forceExecuteRealRelease', 'forcePublishRealPackage',
