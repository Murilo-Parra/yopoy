...
const MODULE_PATH = "src/backend/admin/registerAdminCustomProviderMappingQueryRoutes.ts";
const TEST_PATH = "src/backend/admin/tests/admin-custom-provider-mapping-query-routes-wiring.test.ts";
const REPORT_PATH = "docs/server-refactor-49.1-AD.md";

describe("admin custom provider mapping query route extraction 49.1-AD", () => {
  ...
  it("importa, registra e injeta autenticação e pool dinâmico", () => {
    ...
    expect(serverContent).toMatch(
      /registerAdminCustomProviderMappingQueryRoutes\(app,\s*\{\s*requireMasterAdmin,\s*getPgPool:\s*\(\)\s*=>\s*pgPool\s*\}\);/
    );
    expect(serverContent).not.toMatch(/app\.get\(\s*["']\/api\/admin\/custom-providers\/:id\/mappings["']/);
  });

  it("mantém rotas vizinhas inline e registra a query antes do POST de mappings", () => {
    const serverContent = readProjectFile("server.ts");
    const registerIndex = serverContent.indexOf("registerAdminCustomProviderMappingQueryRoutes(app, {");
    const mappingsPostIndex = serverContent.indexOf('app.post("/api/admin/custom-providers/:id/mappings"');

    expect(serverContent).toContain("registerAdminCustomProviderQueryRoutes(app, {");
    expect(serverContent).toMatch(/app\.post\(\s*["']\/api\/admin\/custom-providers\/:id\/mappings["']/);
    expect(serverContent).toMatch(/app\.get\(\s*["']\/api\/admin\/custom-providers\/:id\/templates["']/);
    expect(registerIndex).toBeGreaterThan(-1);
    expect(mappingsPostIndex).toBeGreaterThan(registerIndex);
  });

  it("preserva método, path, parâmetro, fallback, SQL, payload e erro", () => {
    const routeContent = readProjectFile(MODULE_PATH);

    expect(routeContent).toContain('app.get("/api/admin/custom-providers/:id/mappings"');
    ...
    expect(routeContent).toContain("SELECT * FROM custom_provider_mappings WHERE provider_id = $1");
    expect(routeContent).toContain("`, [id])");
    expect(routeContent).toContain("res.json(result.rows)");
    expect(routeContent).toContain('error: "Falha ao listar mapeamentos"');
  ...
  it("usa interfaces locais mínimas e não importa dependências diretamente", () => {
    ...
    expect(routeContent).toContain("interface AdminCustomProviderMappingQueryPgResult");
    expect(routeContent).toContain("interface AdminCustomProviderMappingQueryPgPool");
    expect(routeContent).toContain("getPgPool: () => AdminCustomProviderMappingQueryPgPool | null");
