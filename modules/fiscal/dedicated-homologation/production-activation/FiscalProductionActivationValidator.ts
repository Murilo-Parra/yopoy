import { FiscalProductionActivationInput } from './FiscalProductionActivationTypes';

export class FiscalProductionActivationValidator {
  public static validate(input: FiscalProductionActivationInput) {
    const blockers: string[] = [];
    const warnings: string[] = [];

    const strJson = JSON.stringify(input || {}).toUpperCase();

    if (strJson.includes('APP.USE(') || strJson.includes('ROUTER.USE(')) {
      blockers.push('app.use/router.use blocked');
    }
