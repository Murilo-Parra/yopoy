...
export class IpmProvider implements INfseProvider {
  name = 'IPM';
  ...
  async validate(config: NfseConfig, data: NfseData): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!config.usuarioApi && !config.certificateId) errors.push('Configuração de autenticação (Usuário ou Certificado) é necessária para IPM.');
    
    // Additional IPM specific validations
    if (data.totalValue <= 0) errors.push('Total value must be greater than zero for IPM.');
    if (!data.customer?.document) errors.push('Customer document is required for IPM.');
  ...
  async generateXml(config: NfseConfig, data: NfseData): Promise<string> {
    ...
    // Simplistic mock skeleton for IPM structure
...
<EnviarLoteRpsEnvio xmlns="http://www.abrasf.org.br/nfse.xsd">
  ...
  async downloadXml(config: NfseConfig, invoiceNumber: string): Promise<string> {
    return '<MockXmlDownloadIpm/>';
  }

  async downloadPdf(config: NfseConfig, invoiceNumber: string): Promise<string> {
    return 'data:application/pdf;base64,mocked_ipm_pdf';
  }
  ...
  // IPM specific methods
  async validateConfiguration(config: NfseConfig): Promise<{ valid: boolean; errors: string[] }> {
    const errors: string[] = [];
    if (!config.certificateId && !config.usuarioApi) errors.push('Certificado Digital ou Usuário API ausente na configuração do provedor IPM.');
    ...
    if (config.environment === 'producao' && !config.urlProducao) errors.push('URL de Produção não informada para ambiente IPM de produção.');
    if (config.environment === 'homologacao' && !config.urlHomologacao) errors.push('URL de Homologação não informada para ambiente IPM de homologação.');
  ...
  getProviderInfo(): any {
    return {
      name: 'IPM',
      description: 'Integração IPM Sistemas',
  ...
  async syncStatus(config: NfseConfig, invoiceNumber: string): Promise<{ status: string; message?: string }> {
    return {
      status: 'SYNCED',
      message: 'Status synced with IPM.'
    };
  }
  ...
  private async mockNetworkCall(url: string, payload: string): Promise<string> {
    return new Promise((resolve) => {
      setTimeout(() => {
         resolve('<Response>IPM Success</Response>');
      }, 500);
    });
  }
}
...
nfseProviderManager.registerProvider('IPM', new IpmProvider());
