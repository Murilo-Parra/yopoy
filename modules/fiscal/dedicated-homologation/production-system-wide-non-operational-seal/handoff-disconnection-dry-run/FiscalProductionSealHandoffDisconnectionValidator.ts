import { FiscalProductionSealHandoffDisconnectionInput } from './FiscalProductionSealHandoffDisconnectionTypes';

export class FiscalProductionSealHandoffDisconnectionValidator {
  public static validate(input: FiscalProductionSealHandoffDisconnectionInput): string[] {
    const blockers: string[] = [];

    const prohibitedKeys = [
      'forceConcludeRealOperationalHandoff',
      'forceCreateRealOperationalContinuation',
    ...
    for (const key of prohibitedKeys) {
      if ((input as any)[key] === true) {
        blockers.push(`The flag ${key} is strictly prohibited in Module 45.4.`);
      }
    }
    ...
    const stringProhibitions = [
      'concluderealoperationalhandoff', 'createrealoperationalcontinuation', 'propagaterealauthority', 'openrealfinalcommandchannel',
      'createrealactivationpath', 'createrealproductionv2path', 'executerealfinalcommand', 'executerealactivationcommand',
      'createrealsystemseal', 'createreallegalrecord', 'createrealoperationalrecord', 'createrealhash', 'createrealsignature',
      'createrealproof', 'createcryptographicproof', 'generaterealpdf', 'generaterealzip', 'generaterealjson', 'generaterealcsv',
    ...
    for (const term of stringProhibitions) {
      if (payloadString.includes(term)) {
        blockers.push(`The term ${term} is strictly prohibited in Module 45.4 payload.`);
      }
    }
