...
export class FiscalRealAuthorizationRequestValidator {
  public static validate(input: FiscalRealAuthorizationRequestInput): { valid: boolean; blockers: string[]; warnings: string[] } {
    ...
    if (jsonStr.includes('approve_production') || jsonStr.includes('approve production')) blockers.push('Request that attempts to approve production detected. Blocked.');
