import { FiscalProductionAuthorizationRequestInput } from './FiscalProductionAuthorizationRequestTypes';

export class FiscalProductionAuthorizationRequestValidator {
  public static validate(input: FiscalProductionAuthorizationRequestInput) {
    ...
    const kwToBlock = [
      ...
      'notifystakeholder', 'submitapproval', 'persistauthorization', 'externalapprover'
    ];

    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('dryrun') && !lowerJson.includes('dry-run') && !lowerJson.includes('simulation') && !lowerJson.includes('plan') && !lowerJson.includes('closure') && !lowerJson.includes('evidence') && !lowerJson.includes('blueprint') && !lowerJson.includes('boundary') && !lowerJson.includes('intake') && !lowerJson.includes('sanitizer') && !lowerJson.includes('envelope')) {
    ...
    const forceKeys = [
      'forcePersistRealAuthorizationRequest', 'forceSubmitToRealStakeholders', 'forceNotifyExternalStakeholder',
      ...
      'forceEnableShadowTraffic', 'forceCreateWorker', 'forceCreateScheduler', 'forceConnectRealDatabase',
