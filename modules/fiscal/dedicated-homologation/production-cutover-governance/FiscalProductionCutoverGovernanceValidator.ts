import { FiscalProductionCutoverGovernanceInput } from './FiscalProductionCutoverGovernanceTypes';

export class FiscalProductionCutoverGovernanceValidator {
  public static validate(input: FiscalProductionCutoverGovernanceInput) {
    ...
    for (const kw of kwToBlock) {
       if (lowerJson.includes(kw)) {
          if (!lowerJson.includes('blueprint') && !lowerJson.includes('contract') && !lowerJson.includes('noop') && !lowerJson.includes('no-op') && !lowerJson.includes('plan') && !lowerJson.includes('criteria') && !lowerJson.includes('matrix') && !lowerJson.includes('window') && !lowerJson.includes('preservation')) {
    ...
    const forceKeys = [
      'forceApproveRealCutover', 'forceExecuteRealCutover', 'forceExecuteGoLive', 'forceActivateProductionV2',
      ...
      'forceDuplicateRequest', 'forceCaptureRequest', 'forceCaptureResponse', 'forceCapturePayload', 'forceModifyAppUse',
      'forceModifyRouterUse', 'forceCallRealEndpoint', 'forceCallLegacyHandler', 'forceCallV2Handler',
      'forceStartRuntimeExecution', 'forceStartCommandQueue', 'forceUnlockRealQueue', 'forceEnqueueRealJob',
      'forceDispatchRealWorker', 'forceCreateWorker', 'forceCreateScheduler', 'forceCreateCron', 'forceRunCommandRunner',
      'forceExecuteShellCommand', 'forceOpenRealTransaction', 'forceCommitRealTransaction', 'forceRollbackRealTransaction',
      'forceConnectRealDatabase', 'forceExecuteDml', 'forceExecuteDdl', 'forceCallRealSefaz', 'forceLoadRealCertificate',
      'forceReadRealPfx', 'forceReadCertificatePassword', 'forceUseRealCrypto', 'forceSignRealXml', 'forceGenerateRealPdf',
