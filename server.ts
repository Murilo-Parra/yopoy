...
// Inicialização segura e resiliente da API do Gemini (Lazy logic)
...
app.post("/api/gemini/parse-receipt", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: error.message || "Erro interno ao processar nota fiscal." });
  }
});

// 2. Chat assistente contábil e logístico empresarial
app.post("/api/gemini/chat-assistant", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: error.message || "Erro no chat corporativo" });
  }
});
...
app.get("/api/nfce", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    const session = await getSessionFromRequest(req);
    if (!session) {
      res.status(401).json({ error: "Sessão expirada para ler notas." });
    ...
    const docs = await getNfceDocuments(session.company_id, { number, series, customer_document, status });
    
    const ip = req.ip || (req.headers["x-forwarded-for"] as string) || "127.0.0.1";
    await logAudit(session.company_id, session.user_id, "CONSULTA_NFCE_LISTA", `Consultou lista de NFC-e com filtros: ${JSON.stringify(req.query)}`, ip);

    res.json({ success: true, documents: docs });
  } catch (err: any) {
    console.error("Erro ao ler nfce_documents:", err);
    res.status(500).json({ error: `Falha ao obter lista de NFC-e: ${err?.message || err}` });
  }
});

// 2. DASHBOARD DE VENDAS NFC-E (MÉTRICAS DO PDV)
app.get("/api/nfce/dashboard", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    const session = await getSessionFromRequest(req);
    if (!session) {
      res.status(401).json({ error: "Sessão expirada." });
    ...
    const docs = await getNfceDocuments(session.company_id);
    ...
    const todayRevenue = todayDocs.reduce((sum, d) => sum + parseFloat(d.total_value || 0), 0);
    ...
    for (const d of todayDocs) {
      ...
      todayProductsCount += items.reduce((sum: number, it: any) => sum + (it.quantity || 1), 0);
    }
    ...
    for (const d of activeDocs) {
      ...
      if (dailyMap[dStr]) {
        dailyMap[dStr].revenue += parseFloat(d.total_value || 0);
    ...
    res.status(500).json({ error: `Erro técnico no dashboard: ${err?.message || err}` });
  }
});

// 3. OBTER DETALHE DE NFC-E POR ID
app.get("/api/nfce/:id", async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    const session = await getSessionFromRequest(req);
    if (!session) {
      res.status(401).json({ error: "Sessão expirada." });
