import { FiscalProductionReentryAttemptDenialInput } from './FiscalProductionReentryAttemptDenialTypes';

export class FiscalProductionReentryAttemptDenialValidator {
  public static validate(input: FiscalProductionReentryAttemptDenialInput): string[] {
    const blockers: string[] = [];

    const prohibitedKeys = [
      'forceCreateRealReentryAttempt',
      'forceUnlockRealResumption',
    ...
    for (const key of prohibitedKeys) {
      if ((input as any)[key] === true) {
        blockers.push(`The flag ${key} is strictly prohibited in Module 46.2.`);
      }
    }
    ...
    const stringProhibitions = [
      'createrealreentryattempt', 'unlockrealresumption', 'resumerealauthority', 'resumerealactivation',
      'resumerealrouting', 'resumerealruntime', 'resumerealdatabase', 'resumerealexternalintegration',
      'resumerealsensitivedataaccess', 'unlockrealgate', 'issurealauthorizationtoken', 'createrealactivationpath',
      'createrealproductionv2path', 'activateproductionv2', 'routetov2', 'disablelegacy', 'changerealtraffic',
      'changerealloadbalancer', 'changerealdns', 'installrealproxy', 'installrealmiddleware', 'installrealtap',
      'activaterealmirror', 'activaterealsniffer', 'activaterealshadowtraffic', 'startrealruntime', 'startrealqueue',
      'enqueuerealjob', 'dispatchrealworker', 'createrealscheduler', 'createrealcron', 'executerealshell',
      'executerealcommandrunner', 'connectdatabase', 'openrealtransaction', 'executedml', 'executeddl', 'migration',
      'migrate', 'callrealsefaz', 'callrealexternalapi', 'sendrealwebhook', 'sendrealslack', 'sendrealwhatsapp',
      'sendrealemail', 'sendrealpager', 'readrealpayload', 'readrealxml', 'readrealpdf', 'readrealtenantdata',
    ...
    for (const term of stringProhibitions) {
      if (payloadString.includes(term)) {
        blockers.push(`The term ${term} is strictly prohibited in Module 46.2 payload.`);
      }
    }
